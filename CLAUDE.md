...
- All REST endpoints under `/api/v1/` (plus `GET /health` at root)
- **CORS** is configured via `CorsConfig.java` — reads `ALLOWED_ORIGIN` env var (defaults to `http://localhost:5173`)
- **Auth (JWT Bearer token):** `POST /api/v1/auth/login`, `POST /api/v1/auth/google`, `GET /api/v1/auth/me`, `POST /api/v1/auth/complete-profile` — see `AuthController`, `JwtAuthFilter`, `JwtService`
- **Predictions:** `GET /api/v1/predictions?competition=…&groupId=…` (tournament matches + user’s predictions for that group), `POST /api/v1/predictions` (upsert) — require authenticated JWT; see `PredictionController`, `TournamentPredictionQueryService`, `PredictionService`
